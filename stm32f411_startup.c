...
void Reset_Handler(void) {
  // Enable CP10 and CP11 full access in the System Control Block
  // enables the FPU
  SCB->CPACR |= ((3UL << (10 * 2)) | (3UL << (11 * 2))); // 0x00F00000

  // 2. Synchronization barriers
  __DSB(); // Complete write to SCB->CPACR
  __ISB(); // Flush pipeline so CPU sees FPU enabled before running float
           // instructions

  // Pointers for .data copy
  uint32_t *p_src_mem = (uint32_t *)&_etext;
  uint32_t *p_dest_mem = (uint32_t *)&_sdata;

  /* Copy .data section from FLASH to SRAM until destination hits _edata */
  while (p_dest_mem < (uint32_t *)&_edata) {
    *p_dest_mem++ = *p_src_mem++;
  }

  // Pointers for .bss zero-init
  p_dest_mem = (uint32_t *)&_sbss;

  /* Set bss section to zero until destination hits _ebss */
  while (p_dest_mem < (uint32_t *)&_ebss) {
    *p_dest_mem++ = 0;
  }
